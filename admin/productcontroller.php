...
?>
	...
	if ($_GET['action'] == 'update') {
		...
		if ($product == "") {
			...
			$type_file = pathinfo($file, PATHINFO_EXTENSION);

			$user_id = $_SESSION['cid'];
			//cek apakah user mau ngubah foto
			...
			if (!$file) {
				...
				// Rename nama file
				$filenew = $id . "_" . $product . "." . $type_file;
				// Set path folder tempat menyimpan fotonya
				$path = "images/" . $filenew;
				$path2 = $_SERVER['DOCUMENT_ROOT'] . "/si_andakar/admin/images/" . $image_before;
				...
				if ($status2 == false) {
					...
					$query = 'UPDATE tblproducts set product_name ="' . $product . '", price ="' . $price . '", image ="' . $nama . '", `category_id`="' . $category . '",`type`="' . $type . '" ,`status`="' . $status . '" WHERE product_id ="' . $id . '"';
					mysqli_query($db, $query) or die(mysqli_error($db));
				}
			}
