		...
		$path = "images/".$fotobaru;
			...
			if ($checkprod > 0) {
              ...
              header("Location: productadd.php?required=quantity");     
				...
				$date = $_POST['date'];
				$user = $_POST['user'];
            	$query = "INSERT INTO `tblproducts`(`product_name`, `quantity`, `date_in`, `category_id`, `supplier_id`, `user_id`, `product_code`, `status`,`image`)
				VALUES ('".$product."','".$quantity."','".$date."','".$category."','".$supplier."','".$user."','".$code."','Available','".$fotobaru."')";
...
<?php
		...
		$qty = $_POST['quantity']; 
			...
			elseif ($supplier == "") {
				header("Location: productupdate.php?required=quantity&id=".$id."");  
			}else{
            	$query = 'UPDATE tblproducts set product_name ="'.$product.'", quantity ="'.$qty.'", `category_id`="'.$category.'",`supplier_id`="'.$supplier.'" WHERE product_code ="'.$id.'"';
				mysqli_query($db, $query) or die(mysqli_error($db));
