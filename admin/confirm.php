...
if (isset($_GET['cancel'])) {
	mysqli_query($db, "UPDATE tbltransac SET status = '2' WHERE transac_code='".$_GET['cancel']."'")or die(mysqli_error($db));
}
elseif (isset($_GET['confirm'])) {
	mysqli_query($db, "UPDATE tbltransac SET status = '1' WHERE transac_code='".$_GET['confirm']."'")or die(mysqli_error($db));
}
