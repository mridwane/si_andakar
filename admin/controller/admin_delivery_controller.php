...
<?php
    header('Location: ../detailtransac_delivery.php?id=' . $no_transac);
}
...
<script type="text/javascript">
    alert(
        'Sukses! Silahkan kirimkan pesanan, pemesan akan mendapatkan notifikasi bahwa pesanan sedang menuju ke alamat tujuan'
    )
</script>
<?php
    header('Location: ../detailtransac_delivery.php?id='.$no_transac);
...
`no_transac`)
VALUES ('" . $datetime . "','" . $nama . "','" . $status_transfer . "', '" . $nominal_customer_trf . "', 'admin','','',
'" . $transac_code . "')";
mysqli_query($db, $query) or die(mysqli_error($db));
}

// update tblbuktitrasnfer untuk bukti transfer customer
$query_update = "UPDATE tblbuktitransfer SET status = 'deny_adm_dp', nominal_trf = '" . $nominal_customer_trf . "',
margin = '" . $margin . "', note = '" . $note . "' WHERE
...
// update tblbuktitrasnfer untuk bukti transfer customer
$query_update = "UPDATE tblbuktitransfer SET status = 'deny_adm_lns', nominal_trf = '" . $nominal_customer_trf . "',
margin = '" . $margin . "', note = '" . $note . "' WHERE
no_transac='" . $transac_code . "' AND user='customer'";
mysqli_query($db, $query_update) or die(mysqli_error($db));
...
?>
<script type="text/javascript">
    window.location.href = 'detailtransac_delivery.php?id=<?php echo $transac_code ?>';
...
if (isset($_POST['save_ketidaksesuaian'])) {
    if ($_POST["status"] == "paid") {
        ...
        // update tblbuktitrasnfer untuk bukti transfer customer
        $query_update = "UPDATE tblbuktitransfer SET status = 'revisi_pelunasan', nominal_trf = '" .
        $nominal_customer_trf . "', margin = '" . $margin . "', note = '" . $note . "' WHERE
            no_transac='" . $transac_code . "' AND user='customer'";
        mysqli_query($db, $query_update) or die(mysqli_error($db));
    ...
    if ($_POST["status"] == "after_revision_lns") {
    $transac_code = $_POST["transac_code"];
    $status = "revisi_pelunasan";
    $note = $_POST["note"];
    $nominal_customer_trf = $_POST["cust_transfer"];
    ...
    $query = "INSERT INTO `tblbuktitransfer`(`date`, `file_name`, `status`, `nominal_trf`, `user`, `margin`, `note`,
    `no_transac`)
    VALUES ('" . $datetime . "','" . $nama . "','" . $status_transfer . "', '" . $nominal_customer_trf . "',
    'admin','','', '" . $transac_code . "')";
    mysqli_query($db, $query) or die(mysqli_error($db));
    }
